...
class DremioReader:
		...
		if self._config.source_process_mode == 'process' or (self._config.pds_process_mode == 'process' and self._config.pds_list_useapi):
			...
			if self._filter.match_source_filter(container):
				...
				if entity is not None:
					...
					if entity['type'] == 'ARCTIC':
						self._read_arctic_children(entity)
	...
	def _read_arctic_children(self, parent_entity):
		self._logger.debug("_read_arctic_children: processing parent entity '" + self._utils.get_entity_desc(parent_entity) + "'")
		if 'entityType' not in parent_entity:
			self._logger.error("_read_arctic_children: bad data, skipping entity: " + self._utils.get_entity_desc(parent_entity))
			return
		for child in parent_entity['children']:
			...
			if child['type'] == "DATASET":
				...
				self._logger.error("_read_arctic_children: not supported entity type " + child['type'])
